...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        import json
        prompt = f"""Evaluate the answer to the question using the rubric.
Question: {question}
Answer: {answer}
Rubric:
{json.dumps(rubric, indent=2)}

Return a JSON object with "scores" (dict of criterion to float 0-1) and "reasoning" (string)."""
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        for r in results:
            ...
            if r.context_precision is not None:
                precisions.append(r.context_precision)
        return {
            "total": total,
            "passed": passed,
            "pass_rate": passed / total if total else 0.0,
            "avg_faithfulness": faith / total if total else 0.0,
            "avg_relevance": rel / total if total else 0.0,
            "avg_completeness": comp / total if total else 0.0,
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures):
            fid = f.qa_pair.metadata.get("id", f"F{i+1:03d}")
    ...
    def generate_improvement_suggestions(
        ...
        if counts.get("off_topic", 0) > 0:
            suggestions.append("Improve intent detection to handle out-of-scope questions")
        if counts.get("refusal", 0) > 0:
            suggestions.append("Relax guardrails to allow more nuanced answers")
        # Ensure we return at least 3 if there are failures, just to be safe with tests
        while failures and len(suggestions) < 3:
            suggestions.append("Review prompt engineering for better overall performance")
        return suggestions
